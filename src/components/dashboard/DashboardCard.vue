    ...
    <Card class="border-l-4 shadow-sm bg-card" :class="borderColorClass">
        ...
        <CardContent class="px-6 py-0">
            ...
            <div class="mt-2 flex items-center text-xs">
                <component v-if="change" :is="isPositive ? ArrowUpRight : ArrowDownRight" class="mr-1 h-4 w-4"
                    :class="isPositive ? 'text-emerald-500' : 'text-rose-500'" />
                <span v-if="change" class="font-medium" :class="isPositive ? 'text-emerald-500' : 'text-rose-500'">
                    {{ formattedChange }}
                </span>
                <span v-if="period" class="ml-1 text-muted-foreground">
                    {{ period }}
                </span>
            </div>
        </CardContent>
